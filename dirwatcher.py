...
def find_magic(filename, starting_line, magic_word):
    """Iterate though each file in the dictionary from the last line that was
       read looking for magic string.
       The last position read in the dictionary is updated.
       Filenames are the key and the value is the starting position used and
       the last read from."""
    line_number = 0
            ...
            if line_number >= starting_line:
                if magic_word in line:
                    logger.info(
                        ...
                        f"found: {magic_word} on line: {line_number + 1}"
                    )
...
def add_file(file_list, ext):
    """Looks to see if files have been added to
    directory by comparing to file list."""
    global files
    for f in file_list:
...
def remove_file(file_list):
    """Looks to see if files have been removed from directory
    by comparing to file list."""
    global files
    for f in list(files):
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Watch for change in directories.")
    parser.add_argument('-i', '--interval', type=float,
                        default=1.0, help="Number of seconds between polling")
    parser.add_argument('-e', '--ext', type=str, default='.txt',
                        help="Filters the type of file to watch for")
...
def main(args):
    ...
    logger.info('Path{}, Ext{}, Interval{}, Magic{}'.format(
        args.directory, args.ext, args.interval, args.magic))
